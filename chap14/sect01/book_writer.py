...
class State(TypedDict):
    ...
    ai_recommendation: str # AI 추천을 저장하는 변수 
    supervisor_call_count: int # supervisor 호출 횟수를 저장하는 함수 
...
def supervisor(state:State):
    ...
    if supervisor_call_count > 2:
        ...
        task = Task(
            ...
            description="supervisor 호출 횟수를 초과했으므로, 현재까지의 진행 상황을 사용자엑 보고한다.",
            done_at=""
        )
    else:
...
def content_strategist (state: State):
    ...
    content_strategist_system_prompt = PromptTemplate.from_template(
        ...
        4. 목차의 논리적인 흐름이 사용자 요구를 충족시키는지 확인하다.
        5. 참고 자료(references)를 적극 활용하여 근거에 기반한 목차를 작성한다. 
        6. 참고 문헌은 반드시 참고 자료(references) 자료를 근거로 작성해야 하며, 최대한 풍부하게 준비한다. URL은 전체 주소로 적어야 한다. 
        7. 추가 자료나 리서치가 필요한 부분을 파악하여 supervisor에게 요청한다. 

        사용자 요구 사항(user_request)을 최우선으로 반영하는 목차로 만들어야 한다. 
        ...
        outline_template은 예시 앞부분만 제시한 것이다. 각 장은 ':---CHAPTER DIVIDER---:'로 구분한다. 
        outline_template:
        {outline_template}
...
def outline_reviewer (state: State):
    ...
    outline_reviewer_system_prompt = PromptTemplate.from_template(
        ...
        - 실제 페이지 URL이 아닌 대표 URL로 되어 있는 경우 삭제해야 함 : 어떤 URL이 삭제되었는지 명시하라.
        - 기타 리뷰 사항 
    ...
    inputs = {
        ...
        'references': references, 
        'messages': messages, 
    } # 현 outline 추가 : communicator가 목차를 몰라 엉뚱한 말하는 것을 방지
...
# 벡터 검색하는 노드 : vector_search_agent
def vector_search_agent(state: State):
    ...
    if task.agent != "vector_search_agent":
        raise ValueError(f"Vector Search Agent가 아닌 agent가 Vector Search를 시도하고 있습니다.. \n {task}")

    vector_search_system_prompt = PromptTemplate.from_template(
        """
        너는 다른 AI들이 수행한 작업을 바탕으로,
        목차(outline) 작성에 필요한 정보를 벡터 검색을 통해서 찾아내는 에이전트다.
    ...
    unique_docs = []
    unique_page_conents = set()
    
    for doc in references['docs']:
        if doc.page_content not in unique_page_conents:
            unique_docs.append(doc)
            unique_page_conents.add(doc.page_content)
    ...
    # # communicator로 보내는 Task 생성 코드 
    # new_task = Task(
    #     agent="communicator",
    #     done=False,
    #     description='AI팀의 진행상황을 사용자에게 보고하고, 사용자의 의견을 파악하기 위해 대화를 나눈다.',
    #     done_at=""
    # )
    ...
    ai_recommendation = "현재 참고자료가 목차를 개선하는 데 충분한지 확인한라. 충분하다면 content_strategist로 목차를 작성하라."
...
def web_search_agent (state: State):
    ...
    if task.agent != "web_search_agent":
        raise ValueError(f"Web Search Agent가 아닌 agent가 Web Search를 시도하고 있습니다.. \n {task}")

    web_search_system_prompt = PromptTemplate.from_template(
        """
        너는 다른 AI들이 수행한 작업을 바탕으로,
        목차(outline) 작성에 필요한 정보를 웹 검색을 통해서 찾아내는 에이전트다.

        현재 부족한 정보를 검색하고, 복합적인 질문은 나누어서 검색하라.
    ...
    msg_str = f"[WEB_SEARCH_AGENT] 다음 질문에 대한 검색 완료: {queries}"
